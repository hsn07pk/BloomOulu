/**
 * /api/tax-certs/[id]/pdf — donor-facing tax certificate PDF download.
 *
 * Mirrors /api/receipts/[number]/pdf:
 *   1. Reads bloomoulu.session cookie + verifies JWT.
 *   2. Calls api `GET /v1/tax-certs/{id}/pdf` with a Bearer token.
 *   3. The api enforces ownership (donor or staff) and 302s to a
 *      /v1/files/<key> URL served from local storage.
...
export async function GET(
  ...
  try {
    const res = await fetch(`${apiBase()}/v1/tax-certs/${encodeURIComponent(id)}/pdf`, {
      method: 'GET',
